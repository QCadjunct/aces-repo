...
# ══════════════════════════════════════════════════════════════════════════════
# PANEL 8 — LangGraph EXC Architecture
# Static architectural diagram of the ACMS Execution Controller.
# Shows planned LangGraph state flow — honest POC roadmap.
#
# Mind Over Metadata LLC — Peter Heller
# ADR-009: D⁴ MDLC Governance — tier_3_execution
# ══════════════════════════════════════════════════════════════════════════════

@app.cell
def _panel8(mo):
    """LangGraph EXC Architecture — static Mermaid diagram + ACMS mapping."""
...
graph TD
    ...
    START --> LOAD[Load skill contract\\nsystem.toon → Pydantic]
    LOAD --> VALIDATE{Validate\\nInputs}
    VALIDATE -->|fail| ERROR_IN([✗ Input Error])
    VALIDATE -->|pass| HOOK_PRE[pre_tool_call.py\\nhook execution]

    HOOK_PRE --> STEP1[Exchange Step 1\\nfabric extract_wisdom\\nGemini 2.5 Flash]
    STEP1 --> STEP2[Exchange Step 2\\nfabric summarize\\nClaude Sonnet 4.5]
    STEP2 --> STEP3[Exchange Step 3\\nfabric extract_insights\\nGemini 2.5 Flash]
    STEP3 --> STEP4[Processing Step\\nfabric synthesize_narrative\\nClaude Sonnet 4.5]
    STEP4 --> PANDOC[Processing Step\\npandoc conversion\\nno LLM]

    PANDOC --> HOOK_POST[post_tool_call.py\\nhook execution]
    HOOK_POST --> COST[Write ADR-009\\ncost entries]
    COST --> VALIDATE_OUT{Validate\\nOutputs}
    VALIDATE_OUT -->|fail| RETRY{Retry\\ncount < max?}
    RETRY -->|yes| STEP1
    RETRY -->|no| ERROR_OUT([✗ Output Error])
    VALIDATE_OUT -->|pass| HOOK_TC[task_complete.py\\nhook execution]
    HOOK_TC --> WORD[WSL2 interop\\nWord auto-launch]
    WORD --> END([⏹ Task Complete])

    style START fill:#1E2761,color:#fff
    style END fill:#0F6E56,color:#fff
    style ERROR_IN fill:#ff6b35,color:#fff
    style ERROR_OUT fill:#ff6b35,color:#fff
    style STEP1 fill:#E6F1FB,color:#1E2761
    style STEP2 fill:#E6F1FB,color:#1E2761
    style STEP3 fill:#E6F1FB,color:#1E2761
    style STEP4 fill:#E1F5EE,color:#0F6E56
    style PANDOC fill:#F7F8FC,color:#64748B
    style HOOK_PRE fill:#FAEEDA,color:#BA7517
    style HOOK_POST fill:#FAEEDA,color:#BA7517
    style HOOK_TC fill:#FAEEDA,color:#BA7517
    style COST fill:#EDE9FE,color:#5B21B6
""")

    # ── PSA elicitation state flow ─────────────────────────────────────────────
    _psa_diagram = mo.mermaid("""
graph TD
    START([▶ New Skill Request])

    START --> PSA[PrincipalSystemArchitect\\nMeta-contract dispatcher]

    PSA --> S1[ACMS_requirements_identity\\nname · persona · domain]
    S1 --> S2[ACMS_requirements_mission\\npurpose · termination · success]
    S2 --> S3[ACMS_requirements_authorities\\ntools · constraints · permissions]
    S3 --> S4[ACMS_requirements_lifecycle\\nhooks · pre/post · task_complete]
    S4 --> S5[ACMS_requirements_cost_model\\nvendor · budget · thresholds]
    S5 --> S6[ACMS_requirements_data\\ninputs · outputs · schemas]

    S6 --> SCORE{Completeness\\nscore = 6/6?}
    SCORE -->|no| FLAG[Flag missing\\nsections]
    FLAG --> PSA
    SCORE -->|yes| SYNTH[SYNTHESIS\\nAssemble system.md]
    SYNTH --> TOKEN{Token count\\n< 800?}
    TOKEN -->|no| TRIM[Trim to\\nBoris Cherney limit]
    TRIM --> TOKEN
    TOKEN -->|yes| SYNC[sync_skill.sh\\ngenerate yaml + toon]
    SYNC --> DEPLOY[Deploy to\\npatterns_custom/]
    DEPLOY --> END([⏹ Skill Deployed])

    style START fill:#1E2761,color:#fff
    style END fill:#0F6E56,color:#fff
    style PSA fill:#1E2761,color:#fff
    style S1 fill:#E6F1FB,color:#1E2761
    style S2 fill:#E6F1FB,color:#1E2761
    style S3 fill:#E6F1FB,color:#1E2761
    style S4 fill:#E6F1FB,color:#1E2761
    style S5 fill:#E6F1FB,color:#1E2761
    style S6 fill:#E6F1FB,color:#1E2761
    style SYNTH fill:#E1F5EE,color:#0F6E56
    style SYNC fill:#FAEEDA,color:#BA7517
    style DEPLOY fill:#EDE9FE,color:#5B21B6
""")

    # ── ACMS → LangGraph mapping table ────────────────────────────────────────
    _mapping = mo.md("""
## ACMS → LangGraph Component Mapping
...
| Exchange Step | `fabric --pattern` invocation | Node function |
| Processing Step | `synthesize_eloquent_narrative` | Node function |
| Execution Controller (EXC) | `fabric_stitch.sh` (current) | `StateGraph.compile()` |
| Task group | `TaskGroup` (planned) | Graph instance |
| Workspace | `cost_audit.log` | Shared state dict |
| Audit trail | ADR-009 entries | State annotations + checkpointer |
| DECforms | Marimo monitor | LangGraph Studio (future) |
| pre_tool_call hook | `hooks/pre_tool_call.py` | Node entry point |
| post_tool_call hook | `hooks/post_tool_call.py` | Node exit point |
| task_complete hook | `hooks/task_complete.py` | `END` node |

## LangGraph State Schema (planned)
...
class ACMSState(TypedDict):
    # Skill contract
    skill_fqsn:      str
    system_toon:     dict          # parsed from system.toon via Pydantic
    
    # Pipeline execution
    run_id:          str           # ADR-009 RUN_ID
    task_group_id:   str           # ADR-009 TASK_GROUP_ID (planned)
    step_outputs:    dict          # step_N -> output text
    current_step:    int
    
    # Cost tracking
    cost_entries:    Annotated[list, operator.add]  # ADR-009 entries
    total_cost:      float
...
## Roadmap — EXC Migration

| Phase | Current | Planned |
|-------|---------|---------|
| **Now** | `fabric_stitch.sh` sequential bash | Working POC, ADR-009 cost tracking |
| **Phase 1** | NATS JetStream events | Real-time monitoring, concurrent task groups |
| **Phase 2** | LangGraph StateGraph | Parallel execution, retry logic, checkpointing |
| **Phase 3** | Distributed cluster | FreedomTower + TheBeast + MiniBeast |
""")

    # ── LangGraph status banner ────────────────────────────────────────────────
    _status = mo.md(
        "⚠ **LangGraph EXC — Architecture POC** — The state flow diagrams show the "
        "planned LangGraph implementation. Current execution uses `fabric_stitch.sh` "
        "(sequential bash). LangGraph migration is Phase 2 on the roadmap."
    ).callout(kind="warn")

    panel8 = mo.vstack([
        mo.md("## 🔗 LangGraph EXC Architecture"),
        mo.md("*ACMS Execution Controller — D⁴ MDLC state flow — Mind Over Metadata LLC*"),
        _status,
        mo.accordion({
            ...
            "PrincipalSystemArchitect Elicitation Flow": mo.vstack([
                mo.md("### Skill creation via PSA — 6-step elicitation + synthesis"),
                _psa_diagram,
            ]),
            "ACMS → LangGraph Mapping + State Schema": _mapping,
        }, multiple=True),
    ])
    return (panel8,)
...
@app.cell
def _assemble(mo, panel1, panel2, panel3, panel4, panel5, panel6, panel7, panel8):
    tabs = mo.ui.tabs({
        "📋 Audit Trail":       panel1,
        "🗂️ Registry":         panel2,
        "🔭 Pipeline":         panel3,
